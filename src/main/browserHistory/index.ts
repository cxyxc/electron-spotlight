...
async function getBrowserHistory(
  ...
  switch (browserName) {
    ...
    case browsers.SEAMONKEY:
      return getMozillaBasedBrowserRecords(
        paths,
        browserName,
        historyTimeLength
      );
    ...
    case browsers.EDGE:
    case browsers.AVAST:
      return getChromeBasedBrowserRecords(
        paths,
        browserName,
        historyTimeLength
      );
    ...
    case browsers.MAXTHON:
      return getMaxthonBasedBrowserRecords(
        paths,
        browserName,
        historyTimeLength
      );
    ...
    case browsers.SAFARI:
      return getSafariBasedBrowserRecords(
        paths,
        browserName,
        historyTimeLength
      );
...
async function getChromeBasedBrowserRecords(
  paths,
  browserName,
  historyTimeLength
) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    const sql = `SELECT title, datetime(last_visit_time/1000000 + (strftime('%s', '1601-01-01')),'unixepoch') last_visit_time, url from urls WHERE DATETIME (last_visit_time/1000000 + (strftime('%s', '1601-01-01')), 'unixepoch')  >= DATETIME('now', '-${historyTimeLength} minutes') group by title, last_visit_time order by last_visit_time`;
...
async function getMozillaBasedBrowserRecords(
  paths,
  browserName,
  historyTimeLength
) {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    const sql = `SELECT title, datetime(last_visit_date/1000000,'unixepoch') last_visit_time, url from moz_places WHERE DATETIME (last_visit_date/1000000, 'unixepoch')  >= DATETIME('now', '-${historyTimeLength} minutes') group by title, last_visit_time order by last_visit_time`;
    await forceWalFileDump(paths[i], tmpFilePaths.db);
    browserHistory.push(
      await getHistoryFromDb(tmpFilePaths.db, sql, browserName)
    );
  }
  deleteTempFiles(newDbPaths);
  return browserHistory;
...
async function getSafariBasedBrowserRecords(
  paths,
  browserName,
  historyTimeLength
) {
  ...
  for (let i = 0; i < paths.length; i++) {
    const tmpFilePaths = copyDbAndWalFile(paths[i]);
    newDbPaths.push(tmpFilePaths.db);
    const sql = `SELECT i.id, i.url, v.title, v.visit_time FROM history_items i INNER JOIN history_visits v on i.id = v.history_item WHERE DATETIME (v.visit_time + 978307200, 'unixepoch')  >= DATETIME('now', '-${historyTimeLength} + " minutes')`;
    await forceWalFileDump(paths[i], tmpFilePaths.db);
    browserHistory.push(
      await getHistoryFromDb(tmpFilePaths.db, sql, browserName)
    );
...
async function getMaxthonBasedBrowserRecords(
  paths,
  browserName,
  historyTimeLength
) {
  const browserHistory = [];
  for (let i = 0; i < paths.length; i++) {
    const sql = `SELECT zlastvisittime last_visit_time, zhost host, ztitle title, zurl url FROM zmxhistoryentry WHERE  Datetime (zlastvisittime + 978307200, 'unixepoch') >= Datetime('now', '-${historyTimeLength} minutes')`;
...
async function getBraveHistory(historyTimeLength = 5) {
  browsers.browserDbLocations.brave = browsers.findPaths(
    browsers.defaultPaths.brave,
    browsers.BRAVE
  );
  return getBrowserHistory(
    browsers.browserDbLocations.brave,
    browsers.BRAVE,
    historyTimeLength
  ).then((records) => {
    return records;
...
async function getAvastHistory(historyTimeLength = 5) {
  browsers.browserDbLocations.avast = browsers.findPaths(
    browsers.defaultPaths.avast,
    browsers.AVAST
  );
  return getBrowserHistory(
    browsers.browserDbLocations.avast,
    browsers.AVAST,
    historyTimeLength
  ).then((records) => {
    return records;
...
async function getMicrosoftEdge(historyTimeLength = 5) {
  browsers.browserDbLocations.edge = browsers.findPaths(
    browsers.defaultPaths.edge,
    browsers.EDGE
  );
  return getBrowserHistory(
    browsers.browserDbLocations.edge,
    browsers.EDGE,
    historyTimeLength
  ).then((records) => {
    return records;
